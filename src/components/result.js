...
const AnimDur = (x) => {
    const durations = [0, 5, 4.5, 4, 3.5, 3, 2.5, 2, 1.5, 1, 0.5];
    if (x === 0) {
        return "0s";
    }
    if (x > 10) {
...
export default function Generate(r) {
    ...
    if (r.data?.cod === 200) {
        ...
        return (
            <div className="h-full w-full sm:h-3/4 md:w-3/4 max-w-3xl rounded-xl z-10 flex flex-wrap text-white text-lg shadow-inset">
                <div className="h-[calc(100%_/_6_-_4px)] w-full bg-black bg-opacity-10 text-2xl flex items-center px-4 mb-[4px] rounded-tl-[inherit] rounded-tr-[inherit] overflow-hidden">
                    {basic.name}, {basic.country}
                    <span className="ml-2 text-lg">
                        (&nbsp;{basic.coord[0]}N,&nbsp;{basic.coord[1]}E&nbsp;)
                    </span>
                    <img src={loader} className={"ml-auto " + (r.loading ? "initial" : "hidden")} alt="loader" />
                    <span className={"ml-auto text-cyan-200" + (r.loading ? " hidden" : " inline-block")}>
                        {r.countDown}
                        <img
                            src={wifi_off}
                ...
                <div className="sm:flex h-5/6 w-full rounded-b-[inherit] relative">
                    <input
                        type="radio"
                        name="cardShift"
                        className="hidden temp-card-radio"
                        checked={r.cardActive % 3 === 0}
                        onChange={() => {}}
                    />
                    <input
                        type="radio"
                        name="cardShift"
                        className="hidden wind-card-radio"
                        checked={r.cardActive % 3 === 1}
                        onChange={() => {}}
                    />
                    <input
                        type="radio"
                        name="cardShift"
                        className="hidden time-card-radio"
                        checked={r.cardActive % 3 === 2}
                        onChange={() => {}}
                    />

                    <div
                        className="h-full w-[calc(100%_/_3_-_2px)] bg-black bg-opacity-10 rounded-bl-[inherit] p-1 divide-y overflow-hidden temp-card"
                        onClick={() => {
                            r.changeCard(0);
                        }}
                    >
                        <div className="h-2/5 w-full relative card-body">
                        ...
                        <div className="h-2/5 w-full flex card-head">
                            <div className="h-full w-1/6 inline-block card-head">
                                <img src={temperature_icon} className="w-full h-full" alt="temperature icon" />
                            </div>
                            <div className="h-full w-5/6 inline-block divide-y card-body">
                                ...
                                <div className="h-2/4 w-full divide-y">
                    ...
                    <div
                        className="h-full w-[calc(100%_/_3_-_4px)] bg-black bg-opacity-10 mx-[4px] p-1 divide-y overflow-hidden wind-card"
                        onClick={() => {
                            r.changeCard(1);
                        }}
                    >
                        <div className="h-1/2 w-full flex card-head">
                            <div className="h-full w-1/2 flex justify-center items-center card-head">
                                <img
                                    src={fan_icon}
                                    className={"w-full h-full animate-spin anim-dur-" + AnimDur(0)}
                                    style={{ animationDuration: AnimDur(wind.speed) }}
                                    alt="fan icon"
                                />
                            </div>
                            <div className="h-full w-1/2 divide-y card-body">
                        ...
                        <div className="h-1/6 w-full flex items-center card-body">
                    ...
                    <div
                        className="h-full w-[calc(100%_/_3_-_2px)] bg-black bg-opacity-10 rounded-br-[inherit] p-1 overflow-hidden time-card"
                        onClick={() => {
                            r.changeCard(2);
                        }}
                    >
                        ...
                        <div className="h-1/5 w-full flex items-center border-t card-body">
                            <div className="h-full w-1/2">
